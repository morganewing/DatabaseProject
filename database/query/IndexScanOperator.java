...
public class IndexScanOperator extends QueryOperator {
  ...
  public int estimateIOCost() throws QueryPlanException {
    ...
    try {
      ...
      reduction = this.transaction.getStats(this.tableName).getReductionFactor(this.columnIndex,
      this.predicate, this.value);
      numIndex = this.transaction.getNumIndexPages(this.tableName, this.columnName);
    } catch (DatabaseException d) {
      throw new QueryPlanException("Query Plan Exception");
    }
    double cost = Math.ceil((reduction * (numIndex + numRecords)));
    return ((int) cost);
  }
