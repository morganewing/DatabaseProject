...
public class QueryPlan {
  ...
  private QueryOperator pushDownSelects(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    /* TODO: Implement me! */
    QueryOperator sOp = source;
    QueryOperator finalOp = source;
    for (int i = 0; i < selectColumnNames.size(); i++) {
      if (i != except) {
        try {
          sOp = new SelectOperator(source, this.selectColumnNames.get(i),
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    // TODO: Implement me!
    SequentialScanOperator sScan = new SequentialScanOperator(this.transaction, table);
    int sCost = sScan.estimateIOCost();
    ...
    if (selectIndices.size() > 0) {
      IndexScanOperator iScan = new IndexScanOperator(this.transaction, table, this.selectColumnNames.get(0),
              this.selectOperators.get(0), this.selectDataBoxes.get(0));
      int iCost = iScan.estimateIOCost();
      for (Integer i : selectIndices) {
        IndexScanOperator currScan = new IndexScanOperator(this.transaction, table, this.selectColumnNames.get(i),
                this.selectOperators.get(i), this.selectDataBoxes.get(i));
        int currCost = iScan.estimateIOCost();
        if (currCost < iCost) {
          iCost = currCost;
          minSelectIdx = i;
          iScan = currScan;
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    QueryOperator minOp = null;
    /* TODO: Implement me! */
    ArrayList costs = new ArrayList<QueryOperator>();
    SNLJOperator sOp= new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    costs.add(sOp.estimateIOCost());
    PNLJOperator pOp = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    costs.add(pOp.estimateIOCost());
    BNLJOperator bOp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    costs.add(bOp.estimateIOCost());
    GraceHashOperator gOp = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    costs.add(gOp.estimateIOCost());
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    for (int i = 0; i < this.joinTableNames.size(); i++) {
      for (Set s : prevMap.keySet()) {
        if (!s.contains(this.joinTableNames.get(i))) {
          String[] right = this.joinRightColumnNames.get(i).split("\\.");
          String[] left = this.joinLeftColumnNames.get(i).split("\\.");
          if (!s.contains(right[0])) {
            if (s.contains(left[0])) {
              for (Set s2 : pass1Map.keySet()) {
                if (s2.contains(this.joinTableNames.get(i))) {
                  QueryOperator leftVal = minCostJoinType(prevMap.get(s), pass1Map.get(s2), left[1], right[1]);
                  QueryOperator rightVal = minCostJoinType(pass1Map.get(s2), prevMap.get(s), right[1], left[1]);

                  Set<String> set = new HashSet<String>(s);
                  set.add(right[0]);

                  if (rightVal.estimateIOCost() < leftVal.estimateIOCost()) {
                    map.put(set, rightVal);
                  } else {
                    if (rightVal.estimateIOCost() > leftVal.estimateIOCost()) {
                      map.put(set, leftVal);
                    } else {
