...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    if (predicate == PredicateOperator.EQUALS) {
      return 1.0f/getNumDistinct();
    }
    if (predicate == PredicateOperator.NOT_EQUALS) {
      return (getNumDistinct() - 1.0f)/getNumDistinct();
    }
    if (predicate == PredicateOperator.GREATER_THAN) {
      return (float) (getMaxValue() - value.getInt())/(getMaxValue() - getMinValue());
    }
    if (predicate == PredicateOperator.GREATER_THAN_EQUALS) {
      return (1.0f/getNumDistinct()) + (float) (getMaxValue() - value.getInt())/(getMaxValue() - getMinValue());
    }
    if (predicate == PredicateOperator.LESS_THAN) {
      return (float) (value.getInt() - getMinValue())/(getMaxValue() - getMinValue());
    }
    if (predicate == PredicateOperator.LESS_THAN_EQUALS) {
      return (1.0f/getNumDistinct()) + (float) (value.getInt() - getMinValue())/(getMaxValue() - getMinValue());
    }
    return 1.0f;
  }
